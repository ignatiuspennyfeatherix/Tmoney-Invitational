...
The displayed fixtures are temporary mock data. The next build stage connects these to Supabase and the Fantasy Premier League feed.
...
Copy `.env.example` to `.env.local` for local development. In Netlify, add `SUPABASE_SECRET_KEY` in the site's environment-variable settings. Do not expose it or commit it to Git.
...
The publishable key is safe for browser use because the database policies in `supabase/schema.sql` limit access. The Supabase secret key is server-only and is accessed only by the scheduled function.

## Fixture sync

`netlify/functions/sync-fixtures.ts` imports Premier League fixtures, completed scores, club crests and current table/form data from the Fantasy Premier League feed every six hours. Run `supabase/migrations/20260816_add_fixture_sync_fields.sql` once in the Supabase SQL editor before deploying. Then add `SUPABASE_SECRET_KEY` to Netlify; the function will run on its schedule after deployment.
